...
const portalLoginSchema = loginSchema.and(z.object({ portal: z.enum(["ADMIN", "MINIAPP"]).optional() }));
...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/auth/login", {
    ...
    if (!hasCurrentPortalAccess(user.portalAccesses, input.portal ?? "ADMIN")) {
      throw new AppError(403, "PORTAL_ACCESS_DENIED", "当前账号未开通该端口或端口权限已到期");
    }
    const sessionUser = toSessionUser(user);
    const token = app.jwt.sign({ sub: user.id, tokenVersion: user.tokenVersion });
    request.sessionUser = sessionUser;
