...
async function coreRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const token = existing || await ensureDemoToken();
  ...
  return <>
    ...
    {open && <div className="ai-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <aside className="ai-drawer" role="dialog" aria-modal="true" aria-label="祥能AI业务助手">
        <header className="ai-drawer-header"><div className="ai-brand-icon"><Bot size={23} /></div><div><strong>祥能AI业务助手</strong><span>Qwen3.5 4B · 本地安全运行</span></div><div className={`ai-health ${health?.status === 'ok' ? 'ok' : 'warn'}`} title="查看健康状态"><Activity size={14} />{health?.status === 'ok' ? '运行正常' : health ? '降级可用' : '检查中'}</div><button type="button" onClick={() => setOpen(false)} aria-label="关闭"><X size={21} /></button></header>
