...
动态业务数据通过权限受控检索实时读取，模型只负责意图、参数和自然语言组织。这样人员状态变化后立即可查询，不需要重新训练模型，也不会让模型直接接触 SQL。
...
| 测试数量 | API 69、后台 30、门户 7、小程序 11、共享包 7、数据导入 4，共 128 |
| 微信小程序构建 | Taro `weapp` 编译成功 |
| `pnpm verify:ollama` | 真实 Qwen 模型语义路由成功，最终实测 1.790 秒 |
