...
- **数据迁移中心**：标准模板、Excel/CSV上传、字段映射、预览校验、错误下载、断点续传和批次撤销；
- **审批流程快速搭建**：按顺序添加审批环节，选择审批人/角色/岗位，配置条件，预览并发布版本；
- **权限开通中心**：端口、角色、数据范围、有效期、逐环节审批、自动开通和精确回收；
- **各端口账户管理**：统一主账号、批量开户、端口访问、密码、微信绑定、强制下线和启停。

操作说明见 [数据迁移中心](docs/operations/data-migration-center.md)、[审批流程快速搭建](docs/operations/workflow-designer.md) 和 [权限与账户中心](docs/operations/access-and-account-center.md)。
